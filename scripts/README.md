...
| `deploy.sh` | 运行模型入口：dev、local、worker、compose-deps、compose-full；`down all` 显式全量停止。 |
| `k8s.sh` | K8s Pod 内运维：配置、PostgreSQL、应用健康、Alembic 状态和手动迁移检查。 |
| `real-flow.sh` | 真实远端依赖验证：通过 SSH tunnel 访问远端开发服务器 Triton，检查 ready、最小 infer，并可复用 Job Platform smoke。 |
