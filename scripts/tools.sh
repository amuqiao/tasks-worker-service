...
常用示例:
  ...
  printf '%s' 'raw-password' | ./scripts/tools.sh env-url postgres --username postgres --host 127.0.0.1 --port 25435 --database tasks_worker_service --password-stdin
  printf '%s' 'raw-password' | ./scripts/tools.sh env-url redis --host 127.0.0.1 --port 26382 --db 0 --password-stdin
...
常用示例:
  printf '%s' 'raw-password' | ./scripts/tools.sh env-url postgres --username postgres --host 127.0.0.1 --port 25435 --database tasks_worker_service --password-stdin
  ./scripts/tools.sh env-url redis --host 127.0.0.1 --port 26382 --db 0
EOF
}
